...
namespace NerdHeadExtensions.Vectors
{
    ...
    public static class RandomV3
    {
        public static Vector3 Range(Vector3 min, Vector3 max)
        {
            return new Vector3(Mathf.Round(UnityEngine.Random.Range(min.x, max.x)), Mathf.Round(UnityEngine.Random.Range(min.y, max.y)), Mathf.Round(UnityEngine.Random.Range(min.z, max.z)));
        }
    }
}
